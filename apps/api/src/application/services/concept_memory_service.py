"""
Phase 0.5: Concept Memory Service.

Tracks per-concept pedagogical teaching history across sessions. For each
concept a student encounters, this records how many times it was taught,
whether explanations succeeded (no misconceptions) or failed, and what
approach was used (procedural vs conceptual). The LearnerContextService
reads this to warn the LLM when a concept has repeatedly failed so it can
...
from src.domain.entities.learning import BLOOM_LEVELS, ConceptMemory
...
_CONCEPTUAL_THRESHOLD = 3  # Bloom rank >= this → conceptual approach
...
class ConceptMemoryService:
    ...
    async def get_struggling_concepts(
        self, user_id: UUID, min_attempts: int = 3
    ) -> list[ConceptMemory]:
        """Returns concepts where the student has struggled (failed > 50% of attempts, >= min_attempts)."""
