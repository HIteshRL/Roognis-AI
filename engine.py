"""
Learning Gap Detector — Roognis AI

Extracts misconceptions from the concept extraction result and promotes them
to persistent learning gaps with severity + confidence scoring.
Gaps auto-escalate on repeated occurrence (medium → high → critical).
"""
...
class LearningGapDetector:
    ...
    async def process_extraction(
        ...
        for misconception in extraction.misconceptions:
            ...
            logger.info("learning_gap_detected", user_id=str(user_id), concept=concept_name,
                        severity=gap.severity, occurrences=gap.occurrence_count)
            updated_gaps.append(gap)

        return updated_gaps

    async def list_gaps(self, user_id: UUID, include_resolved: bool = False) -> list[LearningGap]:
